  ...
  checkBalancedParanthesis.prototype.fn2 = function () {
    ...
    for (let char of str) {
      if (bracket[char]) {
        ...
        stack.length > 0 &&
        bracket[stack[stack.length - 1]] === char
      ) {
        stack.pop();
      } else {
        return false;
...
console.log("BAR", typeof bar, typeof bar.toString(), bar.toString() == bar);
...
function deepClone(object) {
  ...
  for (var key in object) {
    if (typeof object[key] === "object" && !Array.isArray(object[key])) {
      let temp = deepClone(object[key]);
      for (const j in temp) {
        newObject[key + "_" + j] = temp[j];
      }
    } else {
...
// newObject.address.phoneNumber.homePhone = 8103945857;

console.log("new & old", newObject, personalDetail);
